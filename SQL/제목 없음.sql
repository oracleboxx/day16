...
--중복 레코드를 제거하고 업무스를 카운터 하려면 distinct 키워드를 사용한다.
select count(comm) as" 중복을 제거한 업무수" from emp02;
select * from emp02;
...
select deptno ,count(comm)  , count(*) from emp02 group by deptno;
select deptno , avg(sal) from emp02 group by deptno
having avg(sal) >= 2000;

--부서별 최대급여가 1500 이상인 경우 부서번호 ,부서별 최대급여 최소급여가 임시 별칭으로 나오게 검색해보자
select deptno  ,max(sal), min(sal) from emp02 group by deptno
having max(sal) >= 1500;
